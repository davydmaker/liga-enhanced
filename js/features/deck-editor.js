...
(function () {
  ...
  function canHandle() {
    return (
      ...
      !!document.querySelector(
        '.menu-opcoes[id^="menu_"] a[href*="dks/editar"]',
      )
    );
  }
  ...
  function init() {
    ...
    createEditButton();
    enhanceCardLinks();
    loadEditionsMap().catch(function () {
      /* ignore */
    });
  }
  ...
  function getActiveView() {
    ...
    for (var i = 0; i < containers.length; i++) {
      if (
        containers[i].querySelector(".pdeck-block") &&
        containers[i].offsetParent !== null
      ) {
        return containers[i];
      }
    }
  ...
  function parseDeckFromDOM() {
    ...
    lines.forEach(function (line) {
      ...
      var name =
        cardLink.getAttribute("data-lc-name") || cardLink.textContent.trim();
      ...
      cards.push({
        ...
        editionId: editionIdAttr,
        originalEditionId: editionIdAttr,
  ...
  function exitEditMode() {
    ...
    editedCards.forEach(function (card) {
      ...
      if (ediEl) {
        ...
        if (
          card.edition !== card.originalEdition &&
          card.originalEdiHtml !== undefined
        ) {
  ...
  function injectEditControls() {
    editedCards.forEach(function (card, idx) {
      ...
      ctrl.querySelector(".le-dk-minus").addEventListener(
        "click",
        makeHandler(function () {
          changeQty(idx, -1);
        }),
      );
      ctrl.querySelector(".le-dk-plus").addEventListener(
        "click",
        makeHandler(function () {
          changeQty(idx, 1);
        }),
      );
      ctrl.querySelector(".le-dk-remove").addEventListener(
        "click",
        makeHandler(function () {
          toggleRemove(idx);
        }),
  ...
  function buildEditionIconUrl(sigla, rarity) {
    return (
      "//repositorio.sbrauble.com/arquivos/up/ed_mtg/" +
      sigla +
      "_" +
      (rarity || "C") +
      ".gif"
  ...
  function updateEditionIcon(ediEl, info, rarity) {
    ...
    if (info && info.sigla) {
      ...
      if (existingImg) {
        ...
        ediEl.innerHTML =
          '<div class="dk-ed-img"><img title="' +
          escapeAttr(title) +
          '" src="' +
          escapeAttr(iconUrl) +
          '" class="img-ed" height="17" /></div>';
      }
    } else {
      ediEl.innerHTML = '<span class="le-dk-edi-placeholder"></span>';
    }
  ...
  function parseEditionHtml(html) {
    ...
    if (options.length > 0) {
      for (var i = 0; i < options.length; i++) {
        ...
        var sigla =
          opt.getAttribute("data-sigla") || (siglaMatch ? siglaMatch[1] : "");
        var name = siglaMatch
          ? text.replace(/\[([^\]]+)\]\s*/, "").trim()
          : text;
    ...
    var items = doc.querySelectorAll(
      "a[data-id], div[data-id], a[onclick], div[onclick]",
    );
    ...
    fetchEditions(cardId)
      .then(function (editions) {
        ...
        editions.forEach(function (ed) {
          ...
          var displayName =
            ed.name.toLowerCase().indexOf("todas edi") !== -1
              ? "N\u00e3o definida"
              : ed.name;
          ...
          item.addEventListener("click", function (e) {
            ...
            if (isNaoDefinida) {
              ...
              getEditionInfo(ed.id, function (info) {
                updateEditionIcon(
                  cardEdiEl,
                  info || { sigla: ed.sigla, name: ed.name },
                  card.rarity,
                );
              });
            }
  ...
  function addCard(name, section, cardData, selectedEdition, edInfo) {
    ...
    for (var i = 0; i < addedCards.length; i++) {
      if (
        addedCards[i].name === name &&
        addedCards[i].section === section &&
        addedCards[i].edition === edition &&
        !addedCards[i].removed
      ) {
        addedCards[i].qty++;
        renderAddedCards();
        updateChangeCount();
    ...
    addedCards.push({
      ...
      removed: false,
      nameEN:
        cardData && cardData.sNomeIdiomaSecundario
          ? decodeHtmlEntities(cardData.sNomeIdiomaSecundario)
          : "",
  ...
  function renderAddedCards() {
    ...
    activeViewEl
      .querySelectorAll(".le-dk-added-line, .le-dk-added-header")
      .forEach(function (el) {
        el.remove();
      });

    ["main", "side", "maybe"].forEach(function (section) {
      var visible = addedCards.filter(function (c) {
        return !c.removed && c.section === section;
      });
      ...
      var total = visible.reduce(function (s, c) {
        return s + c.qty;
      }, 0);
      ...
      visible.forEach(function (card) {
        ...
        if (edName) {
          editionHtml =
            '<span class="le-dk-edition-name-text">' +
            escapeHtml(edName) +
            "</span>";
        }
        ...
        if (card.imagePath) {
          bindCardTooltip(
            line.querySelector(".le-dk-added-name"),
            card.imagePath,
          );
        }

        line.querySelector(".le-dk-minus").addEventListener(
          "click",
          makeHandler(function () {
            if (card.qty > 1) {
              card.qty--;
        ...
        line.querySelector(".le-dk-plus").addEventListener(
          "click",
          makeHandler(function () {
            card.qty++;
            renderAddedCards();
        ...
        line.querySelector(".le-dk-remove").addEventListener(
          "click",
          makeHandler(function () {
            card.removed = true;
            renderAddedCards();
  ...
  function bindCardTooltip(el, imagePath) {
    el.addEventListener("mouseenter", function (e) {
      ...
      tip.innerHTML =
        ...
        '" width="312" height="445" style="display:block;">';
  ...
  function createSectionSearchBar(section, placeholder) {
    ...
    bar.innerHTML =
      ...
      '<input type="text" class="le-dk-search-input" placeholder="' +
      placeholder +
      '" autocomplete="off">' +
  ...
  function ensureSections() {
    ...
    var mainSearch = createSectionSearchBar(
      "main",
    ...
    if (!sectionInfo.side.headerEl) {
      ...
      if (sectionInfo.maybe.headerEl) {
        sectionInfo.maybe.headerEl.parentNode.insertBefore(
          sideHeader,
          sectionInfo.maybe.headerEl,
        );
      } else {
    ...
    var sideSearch = createSectionSearchBar(
      "side",
    ...
    if (!sectionInfo.maybe.headerEl) {
      ...
      var sideEnd =
        sectionInfo.side.lastEl !== sectionInfo.side.headerEl
          ? sectionInfo.side.lastEl
          : sideSearch;
    ...
    var maybeSearch = createSectionSearchBar(
      "maybe",
      "Adicionar carta ao maybeboard...",
    );
  ...
  function saveDeck() {
    ...
    fetch("/?view=dks/editar&id=" + deckId)
      ...
      .then(function (html) {
        ...
        fd.set(
          "txt_deck_commanderparceiro",
          val("#txt_deck_commanderparceiro"),
        );
        fd.set(
          "txt_deck_commanderparceiro_edicao",
          val("#txt_deck_commanderparceiro_edicao"),
        );
        fd.set("txt_deck_oathbreaker_parc", val("#txt_deck_oathbreaker_parc"));
        fd.set(
          "txt_deck_oathbreaker_parc_edicao",
          val("#txt_deck_oathbreaker_parc_edicao"),
        );
        fd.set("txt_deck_signspell_parc", val("#txt_deck_signspell_parc"));
        fd.set(
          "txt_deck_signspell_parc_edicao",
          val("#txt_deck_signspell_parc_edicao"),
        );
      ...
      .catch(function (err) {
        ...
        if (saveBtn) {
          ...
          saveBtn.textContent = "Salvar Deck";
        }
