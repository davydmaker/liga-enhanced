...
(function () {
  ...
  function canHandle() {
    ...
    const hasProdStock =
      typeof prod_stock !== "undefined" &&
      Array.isArray(prod_stock) &&
      prod_stock.length > 0;
  ...
  function hasActive() {
    return (
      activeFilters.qualid.length > 0 ||
      activeFilters.idioma.length > 0 ||
      activeFilters.extras.length > 0 ||
      activeFilters.edicao.length > 0 ||
      activeFilters.num.length > 0 ||
      activeFilters.lj_uf.length > 0 ||
      activeFilters.lj_tipo.length > 0 ||
      activeFilters.lj_ref.length > 0 ||
      activeFilters.lj_refCount.length > 0 ||
      activeFilters.lj_ref_send.length > 0 ||
      activeFilters.lj_quant.length > 0 ||
  ...
  function enrichStock() {
    ...
    isBuylist =
      allStock.length > 0 && allStock[0].q && typeof allStock[0].q === "object";
    ...
    allStock.forEach((item) => {
      ...
      if (store) {
        ...
        item._refSend =
          store.lj_ref_send !== undefined && store.lj_ref_send !== null
      ...
      if (
        hasStoreData &&
        item._refAvg === undefined &&
        item.lj_ref !== undefined
      ) {
      ...
      item._refCount =
        item.lj_ref !== undefined && item.lj_ref !== null
          ...
          : 0;
      item._quant =
        item.quantFilter !== undefined && item.quantFilter !== null
  ...
  function extractFilterOptions() {
    ...
    if (isBuylist) {
      const qualIds = [...new Set(allStock.flatMap((s) => s._qualities))].sort(
        (a, b) => parseInt(a) - parseInt(b),
      );
      if (qualIds.length > 0) {
        filtersConfig.qualid = {
          label: isProduct ? "Condição" : "Qualidade",
          options: qualIds.map((v) => ({
            id: v,
            label: getQualityLabel(v),
            count: allStock.filter((s) => s._qualities.includes(v)).length,
          })),
        };
      }
    } else {
      ...
      if (qualVals.length > 0) {
        filtersConfig.qualid = {
          label: isProduct ? "Condição" : "Qualidade",
          options: qualVals.map((v) => ({
            id: v,
            label: getQualityLabel(v),
            count: allStock.filter((s) => String(s.qualid) === v).length,
          })),
        };
      }
    }
    ...
    if (isBuylist) {
      const langIds = [...new Set(allStock.flatMap((s) => s._languages))]
        .filter((v) => v !== "0")
        .sort((a, b) => parseInt(a) - parseInt(b));
      if (langIds.length > 0) {
        filtersConfig.idioma = {
          label: "Idioma",
          options: langIds.map((v) => ({
            id: v,
            label: getLanguageLabel(v),
            count: allStock.filter((s) => s._languages.includes(v)).length,
          })),
        };
      }
    } else {
      ...
      if (langVals.length > 0) {
        filtersConfig.idioma = {
          label: "Idioma",
          options: langVals.map((v) => ({
            id: v,
            label: getLanguageLabel(v),
            count: allStock.filter((s) => String(s.idioma) === v).length,
          })),
        };
      }
    }
    ...
    if (!isProduct) {
      ...
      const extraVals = [...new Set(extraRaw.map(String))].sort(
        (a, b) => parseInt(a) - parseInt(b),
      );
      if (extraVals.length > 0) {
        filtersConfig.extras = {
          label: "Extras",
          options: extraVals.map((v) => ({
            id: v,
            label: getExtrasLabel(v),
            count: allStock.filter(
              (s) =>
                String(
                  s.extras === undefined || s.extras === null ? 0 : s.extras,
                ) === v,
    ...
    if (edVals.length > 1) {
      filtersConfig.edicao = {
        ...
        options: edVals
          .map((v) => ({
            id: v,
            label: getEditionLabel(v),
            count: allStock.filter((s) => String(s.idEdicao) === v).length,
          }))
    ...
    const numVals = [
      ...new Set(
        allStock.map((s) => s.num).filter((v) => v && v !== "undefined"),
      ),
    ].sort((a, b) => {
      const na = parseInt(a),
        nb = parseInt(b);
    ...
    if (numVals.length > 1) {
      filtersConfig.num = {
        label: "Numeração",
        options: numVals.map((v) => ({
          id: v,
          label: "#" + v,
          count: allStock.filter((s) => s.num === v).length,
        })),
      };
    }
    ...
    if (ufVals.length > 1) {
      filtersConfig.lj_uf = {
        label: "Estado",
        options: ufVals.map((v) => ({
          id: v,
          label: (C.UF_NAMES[v] || v) + " (" + v + ")",
          count: allStock.filter((s) => s.lj_uf === v).length,
        })),
      };
    }
    ...
    if (hasStores) {
      ...
      const countVerif = allStock.filter(
        (s) => s._selo !== undefined && s._selo >= 2,
      ).length;
      const countNVerif = allStock.filter(
        (s) => s._selo === undefined || s._selo < 2,
      ).length;
      if (countFav > 0)
        tipoOptions.push({ id: "fav", label: "Favoritas", count: countFav });
      if (countFisica > 0)
        tipoOptions.push({
          id: "fisica",
          label: "Com Loja Física",
          count: countFisica,
        });
      if (countVerif > 0)
        tipoOptions.push({
    ...
    if (!hasStores) {
      ...
      for (const range of REF_RANGES) {
        const count = allStock.filter(
          (s) => s._refCount >= range.min && s._refCount <= range.max,
        ).length;
        if (count > 0)
          refCountOptions.push({
            id: range.id,
            label: range.label,
            count: count,
          });
      ...
      if (refCountOptions.length > 0) {
        filtersConfig.lj_refCount = {
          label: "Referências",
          options: refCountOptions,
        };
      }
    }
    ...
    if (sendVals.length > 0) {
      filtersConfig.lj_ref_send = {
        label: "Tempo médio para envio",
        options: sendVals.map((v) => ({
          id: v,
          label: getSendLabel(v),
          count: allStock.filter((s) => s._refSend === v).length,
        })),
      };
    }
    ...
    const prices = allStock
      .map((s) => parseFloat(s.precoFinal))
  ...
  function buildHTML() {
    ...
    html +=
      ...
      "</span> / " +
      allStock.length +
      ' itens <button id="le-clear-all" class="le-clear-btn" style="display:none;">Limpar</button></div>';

    html += '<div class="le-group-title">Filtros do Produto</div>';
    if (filtersConfig.qualid)
      html += ui.buildChipSection("qualid", filtersConfig.qualid);
    if (filtersConfig.idioma)
      html += ui.buildChipSection("idioma", filtersConfig.idioma);
    if (filtersConfig.extras)
      html += ui.buildChipSection("extras", filtersConfig.extras);
    if (filtersConfig.edicao)
      html += ui.buildSearchableList("edicao", filtersConfig.edicao);
    if (filtersConfig.num)
      html += ui.buildSearchableList("num", filtersConfig.num);
    ...
    if (filtersConfig.price) {
      ...
      html +=
        '<input type="number" id="le-price-min" class="le-input le-input-sm le-price-input" placeholder="Min (R$)" min="0" step="0.01">';
      html += '<span class="le-price-sep">—</span>';
      html +=
        '<input type="number" id="le-price-max" class="le-input le-input-sm le-price-input" placeholder="Max (R$)" min="0" step="0.01">';
      html += "</div></div>";
    }

    html += '<div class="le-group-title">Filtros da Loja</div>';
    if (filtersConfig.lj_uf)
      html += ui.buildSearchableList("lj_uf", filtersConfig.lj_uf);
    if (filtersConfig.lj_tipo)
      html += ui.buildChipSection("lj_tipo", filtersConfig.lj_tipo);
    if (filtersConfig.lj_refCount)
      html += ui.buildChipSection("lj_refCount", filtersConfig.lj_refCount);
    if (filtersConfig.lj_ref)
      html += ui.buildChipSection("lj_ref", filtersConfig.lj_ref);
    if (filtersConfig.lj_ref_send)
      html += ui.buildChipSection("lj_ref_send", filtersConfig.lj_ref_send);
    if (filtersConfig.lj_quant)
      html += ui.buildChipSection("lj_quant", filtersConfig.lj_quant);
    ...
    ["edicao", "num", "lj_uf"].forEach((k) => {
      panel.querySelectorAll('.le-cb[data-key="' + k + '"]').forEach((cb) => {
        cb.addEventListener("change", () => {
          ...
          if (cb.checked) {
            if (!activeFilters[k].includes(val)) activeFilters[k].push(val);
          } else {
            activeFilters[k] = activeFilters[k].filter((v) => v !== val);
          }
          ui.reorderList(k);
          applyFilters();
        });
  ...
  function getItemValue(item, key) {
    if (key === "extras")
      return String(
        item.extras === undefined || item.extras === null ? 0 : item.extras,
      );
  ...
  function matchesTipo(item, tipoVal) {
    ...
    if (tipoVal === "verificada")
      return item._selo !== undefined && item._selo >= 2;
    if (tipoVal === "nverificada")
      return item._selo === undefined || item._selo < 2;
    return true;
  }
  ...
  function matchesFilters(item) {
    if (activeFilters.qualid.length > 0) {
      if (isBuylist) {
        if (!activeFilters.qualid.some((q) => item._qualities.includes(q)))
          return false;
      } else {
    ...
    if (activeFilters.idioma.length > 0) {
      if (isBuylist) {
        if (!activeFilters.idioma.some((l) => item._languages.includes(l)))
          return false;
      } else {
    ...
    if (
      activeFilters.edicao.length > 0 &&
      !activeFilters.edicao.includes(String(item.idEdicao))
    )
      return false;
    if (
      activeFilters.num.length > 0 &&
      !activeFilters.num.includes(String(item.num))
    )
      return false;
    if (
      activeFilters.lj_uf.length > 0 &&
      !activeFilters.lj_uf.includes(item.lj_uf)
    )
      return false;
    if (activeFilters.lj_tipo.length > 0) {
      if (!activeFilters.lj_tipo.some((t) => matchesTipo(item, t)))
        return false;
    }
    if (activeFilters.lj_ref.length > 0) {
      const normalized =
        item._refAvg !== undefined && item._refAvg !== null
          ? String(parseFloat(item._refAvg))
          : "";
      if (!activeFilters.lj_ref.includes(normalized)) return false;
    }
    if (activeFilters.lj_refCount.length > 0) {
      if (
        !activeFilters.lj_refCount.some((r) =>
          matchesRefRange(item._refCount, r),
        )
      )
        return false;
    }
    if (
      activeFilters.lj_ref_send.length > 0 &&
      !activeFilters.lj_ref_send.includes(item._refSend)
    )
      return false;
    if (activeFilters.lj_quant.length > 0) {
      const matchesAny = activeFilters.lj_quant.some(
        (q) => item._quant > 0 && item._quant % Number(q) === 0,
      );
      if (!matchesAny) return false;
    }
    const pMin =
      activeFilters.priceMin !== "" ? parseFloat(activeFilters.priceMin) : null;
    const pMax =
      activeFilters.priceMax !== "" ? parseFloat(activeFilters.priceMax) : null;
      ...
      const others = allStock.filter((item) => {
        for (const ok of allKeys) {
          ...
          if (activeFilters[ok].length > 0) {
            if (ok === "lj_tipo") {
              if (!activeFilters[ok].some((t) => matchesTipo(item, t)))
                return false;
            } else if (ok === "lj_quant") {
              const matchesAnyQ = activeFilters[ok].some(
                (q) => item._quant > 0 && item._quant % Number(q) === 0,
              );
              ...
              if (!activeFilters[ok].some((q) => item._qualities.includes(q)))
                return false;
            } else if (isBuylist && ok === "idioma") {
              if (!activeFilters[ok].some((l) => item._languages.includes(l)))
                return false;
            } else {
              if (!activeFilters[ok].includes(getItemValue(item, ok)))
                return false;
            }
          }
        }
      ...
      if (chipKeys.includes(key)) {
        panel
          ...
          .forEach((chip) => {
            ...
            let count;
            if (key === "lj_tipo")
              count = others.filter((s) => matchesTipo(s, val)).length;
            else if (key === "lj_ref")
              count = others.filter(
                (s) =>
                  s._refAvg !== undefined &&
                  s._refAvg !== null &&
                  String(parseFloat(s._refAvg)) === val,
              ).length;
            else if (key === "lj_refCount")
              count = others.filter((s) =>
                matchesRefRange(s._refCount, val),
              ).length;
            else if (key === "lj_ref_send")
              count = others.filter((s) => s._refSend === val).length;
            else if (key === "lj_quant") {
              const q = parseInt(val);
              count = others.filter(
                (s) => s._quant > 0 && s._quant % q === 0,
              ).length;
            } else if (isBuylist && key === "qualid")
              count = others.filter((s) => s._qualities.includes(val)).length;
            else if (isBuylist && key === "idioma")
              count = others.filter((s) => s._languages.includes(val)).length;
            else
              count = others.filter((s) => getItemValue(s, key) === val).length;
  ...
  function updateActiveBar() {
    ...
    const labelMap = {
      qualid: getQualityLabel,
      idioma: getLanguageLabel,
      extras: getExtrasLabel,
    };
    for (const key of ["qualid", "idioma", "extras"]) {
      for (const val of activeFilters[key])
        tags.push({ key, val, label: labelMap[key](val) });
    }
    const tipoLabels = {
      fav: "Favoritas",
      fisica: "Com Loja Física",
      verificada: "Loja Verificada",
      nverificada: "Não Verificada",
    };
    for (const val of activeFilters.lj_tipo)
      tags.push({ key: "lj_tipo", val, label: tipoLabels[val] || val });
    ...
    const refRangeLabels = {
      100: "100 ou mais",
      50: "50 - 99",
      10: "10 - 49",
      0: "Até 9",
    };
    for (const val of activeFilters.lj_refCount)
      tags.push({ key: "lj_refCount", val, label: refRangeLabels[val] || val });
    for (const val of activeFilters.lj_ref_send)
      tags.push({ key: "lj_ref_send", val, label: getSendLabel(val) });
    for (const val of activeFilters.lj_quant)
      tags.push({ key: "lj_quant", val, label: val + " ou mais unidades" });
    for (const val of activeFilters.edicao)
      tags.push({ key: "edicao", val, label: getEditionLabel(val) });
    for (const val of activeFilters.num)
      tags.push({ key: "num", val, label: "#" + val });
    for (const val of activeFilters.lj_uf)
      tags.push({ key: "lj_uf", val, label: val });
    if (activeFilters.priceMin)
      tags.push({
        key: "priceMin",
        val: activeFilters.priceMin,
        label: "Min R$" + activeFilters.priceMin,
      });
    if (activeFilters.priceMax)
      tags.push({
        key: "priceMax",
        val: activeFilters.priceMax,
        label: "Max R$" + activeFilters.priceMax,
      });
    ...
    bar.querySelectorAll(".le-tag").forEach((tag) => {
      tag.addEventListener("click", () => {
        ...
        if (k === "priceMin") {
          activeFilters.priceMin = "";
          const el = document.getElementById("le-price-min");
          if (el) el.value = "";
        } else if (k === "priceMax") {
          activeFilters.priceMax = "";
          const el = document.getElementById("le-price-max");
          if (el) el.value = "";
        } else {
          activeFilters[k] = activeFilters[k].filter((x) => x !== v);
          const chip = document.querySelector(
            '#le-panel .le-chip[data-key="' + k + '"][data-val="' + v + '"]',
          );
          if (chip) chip.classList.remove("le-chip-on");
          const cb = document.querySelector(
            '#le-panel .le-cb[data-key="' +
              k +
              '"][data-val="' +
              v.replace(/"/g, '\\"') +
              '"]',
  ...
  LE.registerModule({
    ...
    init: function () {
      isProduct =
        typeof mpproduct !== "undefined" && typeof prod_stock !== "undefined";
      ...
      const stores = isProduct
        ? typeof prod_stores !== "undefined" && prod_stores
          ? prod_stores
          : {}
        : typeof cards_stores !== "undefined" && cards_stores
          ? cards_stores
          : {};
