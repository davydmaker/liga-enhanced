...
(function () {
  ...
  function canHandle() {
    return typeof bzr !== 'undefined' && !!document.getElementById('card-estoque');
  }
  ...
  function parseFilterGroups() {
    ...
    document.querySelectorAll('.filtro').forEach(function (section) {
      ...
      var groupKey = '';
      if (clearEl) {
        var m = (clearEl.getAttribute('onclick') || '').match(/removeFilters\('([^']+)'\)/);
        if (m) groupKey = m[1];
      }
      ...
      section.querySelectorAll('input[type="checkbox"]').forEach(function (cb) {
        ...
        var count = countEl ? (parseInt(countEl.textContent.replace(/[()]/g, '')) || 0) : 0;
  ...
  function buildHTML() {
    ...
    html += '<div class="le-section"><div class="le-section-title">Busca por Nome</div>';
    html += '<input type="text" id="le-name-search" class="le-input le-input-sm" placeholder="Nome da carta..." autocomplete="off"></div>';
    ...
    for (var i = 0; i < filterGroups.length; i++) {
      ...
      if (isEditions) {
        ...
        html += '<input type="text" class="le-input le-input-sm le-list-search" data-list="le-bzr-list-' + group.groupKey + '" placeholder="Buscar ' + group.title.toLowerCase() + '..." autocomplete="off">';
        html += '<div class="le-artist-list" id="le-bzr-list-' + group.groupKey + '">';
        for (var j = 0; j < group.options.length; j++) {
          ...
          html += '<input type="checkbox" class="le-cb le-bzr-cb" data-native-id="' + opt.cbId + '"' + (checked ? ' checked' : '') + '>';
          html += '<span class="le-artist-name">' + opt.label + '</span>';
          html += '<span class="le-artist-qty" id="le-qty-' + opt.cbId + '">' + opt.count + '</span>';
          html += '</label>';
        }
        html += '</div></div>';
      } else {
        ...
        for (var j = 0; j < group.options.length; j++) {
          ...
          var nativeCb = document.getElementById(opt.cbId);
          var checked = nativeCb && nativeCb.checked;
          var cls = 'le-chip le-bzr-chip' + (checked ? ' le-chip-on' : '') + (opt.count === 0 ? ' le-chip-zero' : '');
          html += '<button class="' + cls + '" data-native-id="' + opt.cbId + '">';
          if (opt.icon) html += opt.icon;
          html += '<span class="le-chip-text">' + opt.label + '</span>';
          html += '<span class="le-chip-qty" id="le-qty-' + opt.cbId + '">' + opt.count + '</span>';
          html += '</button>';
        }
        html += '</div></div>';
      }
  ...
  function updateClearButton() {
    ui.updateNativeClearButton(filterGroups, function () { return nameSearch !== ''; });
  }
  ...
  function updateActiveBar() {
    ...
    bar.innerHTML = tags.map(function (t) {
      return '<span class="le-tag" data-native-id="' + t.cbId + '">' + t.label + ' &times;</span>';
    }).join('');
    ...
    bar.querySelectorAll('.le-tag').forEach(function (tag) {
      tag.addEventListener('click', function () {
        ...
        if (panel) {
          var chip = panel.querySelector('.le-bzr-chip[data-native-id="' + nativeId + '"]');
          if (chip) chip.classList.remove('le-chip-on');
          var cb = panel.querySelector('.le-bzr-cb[data-native-id="' + nativeId + '"]');
          if (cb) {
            cb.checked = false;
            var listEl = cb.closest('.le-artist-list');
            if (listEl) ui.reorderList(null, listEl.id);
  ...
  function hookBzr() {
    ...
    if (typeof bzr.putFilter === 'function') {
      ...
      bzr.putFilter = function (filterId, group, inputName, count, selected) {
        ...
        var chip = document.querySelector('.le-bzr-chip[data-native-id="' + inputName + '"]');
        if (chip) chip.classList.toggle('le-chip-zero', count === 0 && !chip.classList.contains('le-chip-on'));
        var listCb = document.querySelector('.le-bzr-cb[data-native-id="' + inputName + '"]');
        if (listCb) {
          var rowLabel = listCb.closest('.le-artist-row');
          if (rowLabel) rowLabel.classList.toggle('le-artist-zero', count === 0);
        }
      };
    }
