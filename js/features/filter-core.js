...
(function () {
  ...
  function togglePanel() {
    panelOpen = !panelOpen;
    document
      .getElementById("le-panel")
      .classList.toggle("le-panel-open", panelOpen);
    document
      .getElementById("le-backdrop")
      .classList.toggle("le-backdrop-show", panelOpen);
    document
      .getElementById("le-fab")
      .classList.toggle("le-fab-active", panelOpen);
  }
  ...
  function createFloatingButton() {
    ...
    fab.id = "le-fab";
    fab.innerHTML =
      '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="18" x2="20" y2="18"/><circle cx="8" cy="6" r="2" fill="currentColor"/><circle cx="16" cy="12" r="2" fill="currentColor"/><circle cx="10" cy="18" r="2" fill="currentColor"/></svg>';
  ...
  function createPanelShell(bodyHTML) {
    ...
    panel.id = "le-panel";
    panel.innerHTML =
      '<div class="le-panel-header"><h2>Filtros</h2><button id="le-panel-close">&times;</button></div>' +
  ...
  function buildChipSection(key, cfg) {
    ...
    html +=
      ...
      cfg.label +
      ' <span class="le-section-badge" id="le-badge-' +
      key +
      '"></span></div>';
    ...
    for (const opt of cfg.options) {
      ...
      if (key === "iC" && C.COLOR_ICON[opt.id]) {
        ...
        icon =
          '<span class="le-dot" style="background:' + opt.color + '"></span>';
      }
      const qtyHtml = cfg.hideCount
        ? ""
        : '<span class="le-chip-qty">' + (opt.count || 0) + "</span>";
      html +=
        '<button class="le-chip" data-key="' +
        key +
        '" data-val="' +
        opt.id +
        '">' +
        icon +
        '<span class="le-chip-text">' +
        opt.label +
        "</span>" +
        qtyHtml +
        "</button>";
  ...
  function buildSearchableList(key, cfg) {
    ...
    html +=
      ...
      '"></span></div>';
    html +=
      '<input type="text" class="le-input le-input-sm le-list-search" data-list="le-list-' +
      key +
      '" placeholder="Buscar ' +
    ...
    for (const opt of cfg.options) {
      ...
      html +=
        ...
        opt.label.replace(/"/g, "&quot;") +
        '"><input type="checkbox" class="le-cb" data-key="' +
        key +
        '" data-val="' +
        esc +
